...
def main():
    ...
    print(f"{'Method':<15}{'Language':<15} {'Runtime':>15}")
    
    codon_test_output = codon_run().splitlines()
    python_test_output = python_run().splitlines()
    for output in codon_test_output:
        out_tuple = output.split("|")
        print(f"{out_tuple[0]:<15}{out_tuple[1]:<15} {out_tuple[2]:>15}ms")
    for output in python_test_output:
        out_tuple = output.split("|")
        print(f"{out_tuple[0]:<15}{out_tuple[1]:<15} {out_tuple[2]:>15}ms")


        # print(f"{out_tuple[0]:<10} {python_test_output.strip():>6}ms")
...
def python_run():
    result = subprocess.run(f'cd test  && ulimit -s 8192000 && python test_alignment.py', capture_output=True, cwd= base_dir_path, check=True,shell=True, text=True)  
...
def codon_run():
    # print(base_dir_path)
    subprocess.run(f'cd test && codon build -release test_alignment.codon -o test_alignment',cwd= base_dir_path, shell=True, check=True)

    result = subprocess.run(f'cd test && ./test_alignment',cwd= base_dir_path, shell=True, check=True, capture_output=True, text=True)
    return str(result.stdout)
