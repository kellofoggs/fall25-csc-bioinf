...
class LinkedListNode:
    ...
    def __init__(self, content, next=None):
        ...
        self.next:LinkedListNode = next # The link to next node
...
class LinkedList:
    ...
    def append(self, node_content):
        ...
        if self.head is not None and self.tail is not None and self.size != 0:
            ...
            self.tail.next = LinkedListNode(node_content, None)
            self.tail = self.tail.next
            
        else:
            self.head = LinkedListNode(node_content, None)
    ...
    def insert(self, node_content, index: int):
        ...
        if not index >= 0:
            ...
            temp = self.head
            self.head = LinkedListNode(node_content, temp)
    ...
    def find(self, index:int) -> LinkedListNode:
        curr = self.head
        while index > 0:
            curr = curr.next
            index = index - 1
...
class DoublyLinkedNode(LinkedListNode):
    next: LinkedListNode
    prev: LinkedListNode
    pass
...
class SinglyLinkedNode(LinkedListNode):
    next: LinkedListNode
